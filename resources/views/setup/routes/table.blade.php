        ...
        <tbody class="table-border-bottom-0">
            @foreach ($routes as $key => $value)
                <tr>
                    ...
                    <td>
                        <div>
                            <button type="button" data-id="{{ $value->id }}" data-name="{{ $value->name }}" data-price="{{ $value->price }}" data-status="{{ $value->status }}" class="btn btn-icon btn-outline-primary editItem"
